...
const EmailTemplateEditor = ({ onChange }) => {
  ...
  return (
    <div className="border border-gray-200 rounded-md overflow-hidden">
      <div className="flex items-center gap-1 px-2 py-1.5 border-b border-gray-200 bg-white flex-wrap">
        ...
        {rowButtons.map(({ cmd, label, id, val }) => (
          <button
            key={id}
            type="button"
            onMouseDown={(e) => {
      ...
      <style>{`.email-rich-editor h1{font-size:1.5rem !important}
.email-rich-editor h2{font-size:1.25rem !important}
.email-rich-editor h3{font-size:1.125rem !important}
.email-rich-editor p{font-size:0.875rem !important}
.email-rich-editor blockquote{border-left:3px solid #6A73FC;padding-left:0.75rem;color:#737B8B}`}</style>
      ...
      <div
        ...
        data-placeholder="Write your email template..."
        className="email-rich-editor bg-[#F5F5F5] min-h-[220px] px-4 py-3 text-sm text-gray-800 outline-none leading-relaxed empty:before:content-[attr(data-placeholder)] empty:before:text-gray-400"
      />
    </div>
  );
};
